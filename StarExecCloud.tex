...
A Python script {\tt runsolver.py} is available to run a containerized ATP system from the 
command line.
The script is shown in Appendix~\ref{runsystem}.
Minimally the script must have the container name (JACK IS THAT A MISNOMER IN THE SCRIPT?)
as a command line argument.
By default the script starts the container in {\tt podman}, taking the problem from {\tt stdin},
imposing a CPU limit and wall clock limit of 60s, imposing no memory limit, asking the ATP system
to try prove the problem's conjecture is a theorem, by executing the container's 
{\tt run\_container} script with the problem, CPU limit, wall clock limit, memory limit, and
the proof request as the script's arguments.
All the components can be changed via command line options to {\tt runsolver.py}.
The problem file is passed into the container using Podman volume mounting, copying the
problem file to {\tt /artifacts/CWD/benchmark} inside the executing container.
